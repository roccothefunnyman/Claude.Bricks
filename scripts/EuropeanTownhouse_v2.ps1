...
# Plate row along X with gaps (for floor separation behind windows)
...
# FIX v2: front wall row (Z=10) uses gaps to avoid plates behind windows
L "0 // Floor separation"
foreach ($yp in @(-156, -164)) {
    # Front row with gaps at openings
    PlateRowX $LtGray 0 640 $yp $FZ $allFrontGaps
    # All other rows: full fill (Z=30 onward)
    for ($z = 30; $z -le 630; $z += 20) {
        FillXPlate $LtGray 0 640 $yp $z
    }
}

# --- ACCENT BAND between floors (trim color on front wall) ---
L "0 // Accent band"
FillXTile $LtGray 0 $DivX -156 ($FZ - 10)    # left section overhang
FillXTile $LtGray $DivX 640 -156 ($FZ - 10)   # right section overhang
...
# --- BALCONY ---
# FIX v2: only overhang plates (Z=-30, Z=-10), Z=10 is covered by floor sep
L "0 // Balcony"
...
# Window boxes below first floor windows (at floor sep level, in front of wall)
...
# --- LEFT SECTION ROOF CAP (Tan, slightly lower than right) ---
...
# FIX v2: Right side wall parapet (1 brick)
FillZ $LtGray 0 640 $parapetY1 $RX

# FIX v2: Right section back parapet (1 brick)
FillX $LtGray $DivX 640 $parapetY1 $BZ

# --- DECORATIVE CORBELS ---
# FIX v2: moved to Z=-10 to avoid collision with roof plates at Z=10
# R0 rotation shows triangular bracket profile from front
L "0 // Corbels"
...
# FIX v2: Left side wall parapet (1 brick)
FillZ $Tan 0 640 $leftParY $LX

# FIX v2: Left section back parapet (1 brick)
